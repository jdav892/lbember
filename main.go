...
//returns the attmempts for request
...
func isBackendAlive(u *url.URL) bool {
  ...
  if err != nil {
    ...
    defer conn.Close()//close it as we don't need to maintain the connection
    return true
}

//healthCheck runs a routine for check status of backends every 2 minutes
func healthCheck() {
...
func main() {
  ...
  for _, tok := range tokens {
    ...
    proxy.ErrorHandler = func(writer http.ResponseWriter, request *http.Request, e error){
      ...
      log.Printf("%s(%s) Attmepting to retry %d\n", request.RemoteAddr, request.URL.Path, attempts)
